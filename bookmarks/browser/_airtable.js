...
const BookmarksAPI = {
      ...
      .service-32 .btn-md {
        ...
        .Selector input {
          background-color: rgba(50,50,50,0.8);
            ...
            /* Sizes */
                height:auto;border-radius: 6px; background-size: cover; padding:4px 24px; margin: 8px;
        }
        /* Hidden */
  ...
  fillSelectMenu:function(){
    ...
    BookmarksAPI.tagNames.forEach((tagname, i) => {
        ...
        const selectorSlug = BookmarksAPI.slugify(tagname)
        selectorEl.type = "radio"
        selectorEl.setAttribute("name", "selector")
        selectorEl.setAttribute("title", tagname)
        selectorEl.dataset.name = tagname;
  ...
  update:function(){
    ...
    if (gridBox) {
      if (this.state === "All Star"){
        this.records.forEach(r => (r.fields.fav === true) && gridBox.appendChild(this.createRecordEl(r)))
      }
      else {
  ...
  createRecordEl:function(rec){
    ...
    box.innerHTML = `
        <cbs-card
            ...
            tag="${rec.fields.tag}"
            tagColor="${BookmarksAPI.getTagColor(rec.fields.tag)}"
        ></cbs-card>
    `
    return box
  },
  ...
  getTagColor:function(tagName){
    //console.log("asd",BookmarksAPI.colors[BookmarksAPI.tagNames.indexOf(tagName)])
    const tagColor = (tagName === "All Star" )
      ? "rgba(50,50,50,0.8)" 
      : BookmarksAPI.colors[BookmarksAPI.tagNames.indexOf(tagName)]

    return tagColor
  },
  getActiveColor:function(){
    //console.log("active: ",BookmarksAPI )
    if (BookmarksAPI.state === "All Star") return "rgba(50,50,50, 0.8);"
    else {
      const tagIndex = BookmarksAPI.tagNames.indexOf(BookmarksAPI.state)
      const tagColor = BookmarksAPI.colors[tagIndex]
      //console.log("color", tagColor)
      BookmarksAPI.activeColor = tagColor
    }
  }
};
...
(async function(){
  ...
  if (tagdata && tagdata.length > 0){
    const tagColors = tagdata.map(t => t.fields.color || t.color)
    BookmarksAPI.colors = tagColors;
